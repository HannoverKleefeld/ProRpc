...
class RPCBuilder extends RPCDevice{
	...
	function __construct($deviceJsonConfigFileName=null, $cachePath=null){
	...
	public function AddParamDefaultValue($paramName, $value){
		if(is_array($value)){
	...
	public function AddToCache($source, $host, $port=0){
	...
	public function Create($url_or_filename,$ReturnResult=false, $ClearBefore=false){
...
//		$this->debug(DEBUG_BUILD, "Options: ".static::OptionNames($this->_device[DEVICE_CONFIG][CONFIG_PROPS_OPTIONS]),500);
	...
	public static function Discover($timeout=null,$searchIp=null, $searchPort=null){
...
//		$request .= "ST:ssdp:rootdevice\r\n";
	...
	public static function LastNameFromString($s){
		$t=explode(':', $s);
...
//var_export($args);
	...
	private function _loadContent($filename, $force=false){
	...
	private function _importDevice($filename){
...
// var_dump($DataType, $value);					
	...
	private function _dataTpeXMLtoRCP($DataType){
	...
	private function _loadSCPDURL($SCPDURL, $Lowernames){
