...
abstract class RPCConnection {
	...
	function __construct(array $creditials, int $ConnectionType){
		$this->_creditials=$creditials;	
		$this->_connectionType=$ConnectionType;
	}
	abstract public function Execute(string $url,string $serviceID,string $functionname,array $arguments, array $filter=null);
	public function ConnectionType(){return $this->_connectionType;}
	public function SendPacket(string $url, string $content ){
		$p=parse_url($url);
	...
	public static function CreatePacket(string $Method, $Url='/', array $Arguments=null, string $Content=null){
	...
	private function decodePacket(string $Result){
...
class RPCSoapConnection extends RPCConnection{
	public function Execute(string $url,string $serviceID,string $functionname,array $arguments, array $filter=null){
		$this->clearError();
		$params=array(
				'location' 	 => $url,
				'uri'		 => $serviceID,
...
class RPCDevice {
	...
	function __construct(string $deviceJsonConfigFileName=null, string $user=null, string $pass=null, string $caFile=null){
 ...
 */	
	public function FunctionList($ResultReturn=false, string $ServiceName=null){
	...
	public function GetFunctionParam(string $functionName, string $paramName, string $ServiceName=null){
	...
	public function FunctionExist(string $FunctionName, string $ServiceName=null){
	...
	public function GetFunction(string $functionname,string $ServiceName=null){
...
// 			if($service && $list[$functionname]!=$service)return null;
 	...
 	public static function OptionNames(int $Options){
 	...
 	public static function ConnectionTypeName(int $ConnectionType){
  ...
  */ 
 	...
 	public function GetEventServiceNames(){
  	...
  	public function RegisterEvent(string $Service, string $CallbackUrl, $RunTimeSec=300){
  		if(empty($Service))
...
//  		if(empty($result['SID']))return $this->error(ERR_InvalidResposeSID);
	...
	protected function createHelp(array $function, string $functionDisplayName, int $HelpMode, $HelpWidht = 80){
		$paramdesc=null;
		if(!empty($function[FUNCTION_PARAMS][PARAM_IN])) {
	...
	protected function getDescription(int $DescriptionID, $WordWarp=0){
	...
	private function _callSource(string $FunctionName, string $Source, array $Arguments, $filter=null){
...
//  		set_error_handler($old_handler);
	...
	private function getUrl(int $port=null, string $path=null){
...
//if($this->_lastFunction[FUNCTION_NAME]=="RemoteControl") exit(var_dump($param,$value));
	...
	private function getKeyCode(int $KEY_code){
