...
class IPSRpcGateway extends IPSBaseModule  {
	...
	function RequestAction($Ident, $Value){
		if($Ident=='PROCESS_UPDATE'){
			...
			return true;
		}else parent::RequestAction($Ident, $Value);
	}
	function ForwardData($JSONString){
		$this->SendDebug(__FUNCTION__,$JSONString,0);
 		...
 		if($this->_handleInternalFunction($function, $arguments,$result, $data['ObjectID']))	return $result;
	...
	protected function sendDataToClients(string $Command, $Data, $SenderObjectID=0){
		$data['Buffer']=['Command'=>$Command,'Data'=>$Data];
		$data['DataID']='{19650302-CONT-MAJA-PRPC-20180101XLIB}';
		$data['ObjectID']=empty($SenderObjectID)?$this->InstanceID:$SenderObjectID;
		$data=json_encode($data);
		$this->SendDebug(__FUNCTION__,$data,0);
		$this->SendDataToChildren($data);
	}
	protected function _handleInternalFunction($function, $arguments,&$result, $SenderObjectID){
		...
		if($function=='DataChanged'){
			$this->sendDataToClients('DataChanged', $arguments, $SenderObjectID);
			$result=$this->_returnResult(true);
			return true;
		}
		return false;
	}
	...
	protected function _loadConfigFile($Filename){
		...
		if($Filename==$this->GetBuffer('ConfigFile'))return $this->_applyConfig();
